...
let AcceptRejectComponent = class AcceptRejectComponent {
...
AcceptRejectComponent.ctorParameters = () => [
...
let AppComponent = class AppComponent {
    constructor(location, commonFunctions) {
...
/* harmony import */ var ng2_pdf_viewer__WEBPACK_IMPORTED_MODULE_27__ = __webpack_require__(/*! ng2-pdf-viewer */ "./node_modules/ng2-pdf-viewer/fesm2015/ng2-pdf-viewer.js");
...
let AuthorizationComponent = class AuthorizationComponent {
    constructor(stepperService, authService, router, service, commonFunctions) {
    ...
    ngOnInit() {
        ...
        this.service.getAuth(apiUniqueKey).subscribe(res => {
            ...
            if (res['ErrorCode'] == 200) {
                ...
                this.authService.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_6__["AlertMessages"].SOMETHING_WRONG);
    ...
    authorization(option_val, i, name) {
        this.index = i;
    ...
    generateOtp() {
        this.loading = true;
        let apiUniqueKey = new Date().getTime().toString();
        this.stepperService.auth_reinit2(apiUniqueKey, 3, '').subscribe(response => {
            this.loading = false;
...
let AuthService = class AuthService {
    constructor(router, snackBar, tokenStorage, common) {
        this.router = router;
    ...
    alertToUser(message, logout = false, isError = false) {
        const snackbarConfig = new _angular_material__WEBPACK_IMPORTED_MODULE_3__["MatSnackBarConfig"]();
...
let TokenStorage = class TokenStorage {
    ...
    getAccessToken() {
        ...
        const token = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].ACCESS_TOKEN);
    ...
    /**
       ...
       */
    getBearerToken() {
        const token = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].BEARER_TOKEN);
        return token;
    }
    /**
      * Get session Id
      ...
      */
    getSessionId() {
        const sessionId = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].SESSION_ID);
        return sessionId;
    }
    /**
       * Get sr Id
       ...
       */
    getSrId() {
        const srId = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].SR_ID);
        return srId;
    }
    /**
       * Set sr token
    ...
    setRouteData(key, value) {
        if (value) {
            const valueEnc = crypto_js__WEBPACK_IMPORTED_MODULE_4__["AES"].encrypt(JSON.stringify(value), this.ENCRYPTION_KEY).toString();
            return sessionStorage.setItem(key, valueEnc);
        }
        return null;
    }
    setParamData(key, value) {
        if (value) {
            const valueEnc = crypto_js__WEBPACK_IMPORTED_MODULE_4__["AES"].encrypt(JSON.stringify(value), this.ENCRYPTION_KEY).toString();
            return sessionStorage.setItem(key, valueEnc);
        }
    ...
    getRouteData() {
        try {
            const routeData = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].ROUTES);
            if (routeData) {
                const bytes = crypto_js__WEBPACK_IMPORTED_MODULE_4__["AES"].decrypt(routeData, this.ENCRYPTION_KEY);
                const routeDataDec = JSON.parse(bytes.toString(crypto_js__WEBPACK_IMPORTED_MODULE_4__["enc"].Utf8));
                return routeDataDec;
            }
        }
        catch (err) {
    ...
    getParamData() {
        try {
            const paramData = sessionStorage.getItem(src_app_app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].PARAM_DATA);
            if (paramData) {
                const bytes = crypto_js__WEBPACK_IMPORTED_MODULE_4__["AES"].decrypt(paramData, this.ENCRYPTION_KEY);
                const paramDataDec = JSON.parse(bytes.toString(crypto_js__WEBPACK_IMPORTED_MODULE_4__["enc"].Utf8));
                return paramDataDec;
            }
        }
        catch (err) {
...
let BaseAPIService = class BaseAPIService {
    ...
    post(workflowId, params) {
        let SERVER_URL = this.baseUrl + `d/workflows/${workflowId}/${src_environments_environment__WEBPACK_IMPORTED_MODULE_3__["environment"].apiVersion}/execute`;
        return this.http.post(`${SERVER_URL}`, JSON.stringify(params));
    }
    postUpload(params) {
        let SERVER_URL = this.baseUrl + 'd/drive/upload';
        const headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]().set(src_app_app_constant__WEBPACK_IMPORTED_MODULE_4__["Constants"].InterceptorSkipHeader, '');
...
let CommonService = class CommonService {
    constructor(baseAPIService, tokenStorage) {
...
let FilterInterceptor = class FilterInterceptor {
    constructor(router, auth, tokenStorage, encryptionService, dialog) {
        this.router = router;
        this.auth = auth;
        this.tokenStorage = tokenStorage;
    ...
    intercept(request, next) {
        ...
        if (!request.headers.has(_app_constant__WEBPACK_IMPORTED_MODULE_9__["Constants"].InterceptorSkipHeader) && request.method != "GET") {
            let encryption = this.encryptionService.encrypt((request.body), src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["environment"].aesPublicKey);
            request = request.clone({
            ...
            const headers = request.headers.delete(_app_constant__WEBPACK_IMPORTED_MODULE_9__["Constants"].InterceptorSkipHeader);
        ...
        return next.handle(request).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(event => {
            if (event instanceof _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpResponse"]) {
                ...
                if (this.decryptResponse(event)) {
                    ...
                    if (event.headers.get('ACCESS_TOKEN')) {
                        this.tokenStorage.setBearerToken(event.headers.get('ACCESS_TOKEN'));
                    }
                    if ('body' in event &&
                        'login_required' in event['body'] &&
                        event['body']['login_required'] === true) {
                        this.auth.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].SESSION_EXPIRED);
            ...
            if (error instanceof _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpErrorResponse"]) {
                ...
                if ('body' in event &&
                    ...
                    this.auth.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].SERVER_ERROR);
                }
            }
        }));
    }
    decryptResponse(event) {
        ...
        if (timestamp != null) {
            let decryption = this.encryptionService.decrypt(randomkey, timestamp, responseHash, event.body, src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["environment"].aesPublicKey);
            return decryption;
        }
        else {
            return false;
...
FilterInterceptor.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_3__["Router"] },
...
let CustomerComponent = class CustomerComponent {
    constructor(dialog, baseAPIService, tokenStorage, customerService, sanitizer, router, service, _snackBar) {
        this.dialog = dialog;
        this.baseAPIService = baseAPIService;
        this.tokenStorage = tokenStorage;
        ...
        if (this.tokenStorage.getAccessToken()) {
            ...
            this.customerService.cust(this.apiUniqueKey).subscribe(res => {
                ...
                if (this.sr_type == 1008) {
                    ...
                    this.old_Dob = this.res_['ProcessVariables']['dobUpdate']['oldDob'] ? this.res_['ProcessVariables']['dobUpdate']['oldDob'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                    this.new_dob = this.res_['ProcessVariables']['dobUpdate']['newDob'] ? this.res_['ProcessVariables']['dobUpdate']['newDob'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                }
                if (this.sr_type == 1006) {
                    console.log("in email Sr");
                    this.old_email = this.res_['ProcessVariables']['emailUpdate']['maskedOldEmail'] ? this.res_['ProcessVariables']['emailUpdate']['maskedOldEmail'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                    this.new_email = this.res_['ProcessVariables']['emailUpdate']['newEmail'] ? this.res_['ProcessVariables']['emailUpdate']['newEmail'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                ...
                if (this.sr_type == 1007) {
                    ...
                    this.old_mobile_number = this.res_['ProcessVariables']['custDetails']['mobileNumber'] ? this.res_['ProcessVariables']['custDetails']['mobileNumber'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                    this.new_mobile_number = this.res_['ProcessVariables']['mobileUpdate']['newMobile'] ? this.res_['ProcessVariables']['mobileUpdate']['newMobile'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
                }
    ...
    createThumbnail(docID, type) {
        if (docID != "null") {
            ...
            this.mySubscription = this.baseAPIService.getImage(docID).subscribe(res => {
                ...
                let currentImgUrl = this.sanitizer.bypassSecurityTrustUrl(unsafeImageUrl);
                if (type == 1) {
    ...
    getDropDownOptions(srtype) {
        this.loading = true;
        this.customerService.getOptions(srtype).subscribe(res => {
            this.loading = false;
    ...
    dob_Primary_backImg() {
        ...
        this.loading = true;
        this.customerService.uploadImg(this.img4, this.dob_backimg_Name).subscribe(res => {
            this.loading = false;
            if (res['login_required'] == true) {
                this.errorPage();
                return;
    ...
    uploadImg1() {
        ...
        this.loading = true;
        console.log('Caught');
        this.customerService.uploadImg(this.img1, this.panImgName).subscribe((event) => {
    ...
    uploadImg2() {
        ...
        this.loading = true;
        this.customerService.uploadImg(this.img2, this.frontImg).subscribe((event) => {
    ...
    uploadImg3() {
        ...
        this.loading = true;
        this.customerService.uploadImg(this.img3, this.backImg).subscribe((event) => {
    ...
    uploadToServer_Default() {
        ...
        this.loading = false;
        this.mySubscription = this.customerService.uploadImg(this.front_file[0], this.front_file[0].name).subscribe((event) => {
            // this.isDefault = true;
            ...
            this.frontImg = '';
            this.pdf1 = false;
            this._snackBar.open(_app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].SOMETHING_WRONG, 'Error', {
                duration: 4000,
            });
            return;
        });
    ...
    uploadToServer_back() {
        ...
        this.loading = false;
        this.mySubscription = this.customerService.uploadImg(this.back_file[0], this.back_file[0].name).subscribe((event) => {
            // this.isDefault = true;
            console.log("uploadToServer", event);
            switch (event.type) {
                case _angular_common_http__WEBPACK_IMPORTED_MODULE_12__["HttpEventType"].Sent:
    ...
    processResopnse() {
        ...
        let old_pan_number = this.res_['ProcessVariables']['panUpdate']['oldPan'] ? this.res_['ProcessVariables']['panUpdate']['oldPan'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
        let new_pan_number = this.res_['ProcessVariables']['panUpdate']['newPan'] ? this.res_['ProcessVariables']['panUpdate']['newPan'] : _app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].NA_BANK_MSG;
    ...
    callDalog(popupMsg) {
        ...
        const dialogRef = this.dialog.open(_core_components_dialog_box_dialog_box_component__WEBPACK_IMPORTED_MODULE_11__["DialogBoxComponent"], dialogConfig);
...
let CustomerService = class CustomerService {
    ...
    reject(approved, reason, apiUniqueReqId) {
        let processVariables = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].PROJECT_ID,
            "workflowId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Accept.WORKFLOW_ID,
            "processId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Accept.PROCESS_ID,
...
let MydirectDirective = class MydirectDirective {
    constructor(el) {
        this.el = el;
    }
    ngAfterContentInit() {
...
MydirectDirective.ctorParameters = () => [
...
let EmailverifyComponent = class EmailverifyComponent {
    constructor(authService, commonFunctions, emailverifyService, tokenStorage, service, route, router, _snackBar) {
        this.authService = authService;
        this.commonFunctions = commonFunctions;
    ...
    getSessiondetails() {
        this.route.queryParamMap.subscribe(params => {
            let details = Object.assign({}, params.keys, params);
            ...
            if (response['status']) {
                if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                    if (response['payload']['processResponse']['ProcessVariables']['authRefId']) { // set auth token
                        ...
                        this.authService.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_11__["AlertMessages"].SOMETHING_WRONG);
                        this.commonFunctions.showErrorPage();
                    }
                }
                else {
    ...
    emailVerify() {
        this.loading = true;
        this.apiUniqueKey = new Date().getTime().toString();
        this.emailverifyService.verifyemail(this.refId, this.apiUniqueKey).subscribe(response => {
            this.loading = false;
            if (response['status']) {
                if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
...
let EmailverifyService = class EmailverifyService {
    constructor(baseAPIService, tokenStorage) {
        this.baseAPIService = baseAPIService;
        this.tokenStorage = tokenStorage;
    }
    authEmail(apiUniqueReqId) {
        let body = {
    ...
    verifyemail(refid, apiUniqueReqId) {
        let body = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_4__["Constants"].PROJECT_ID,
...
EmailverifyService.ctorParameters = () => [
...
let MobileVerifyComponent = class MobileVerifyComponent {
    constructor(authService, commonFunctions, formBuilder, tokenStorage, mobileVerifyService, router, route, service) {
        this.authService = authService;
        this.commonFunctions = commonFunctions;
            ...
            if (response['status']) {
                if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                    if (response['payload']['processResponse']['ProcessVariables']['authRefId']) { // set auth token
                        ...
                        this.authService.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_9__["AlertMessages"].SOMETHING_WRONG);
                        this.commonFunctions.showErrorPage();
                    }
                }
                else {
    ...
    submitOtp() {
        this.loading = true;
        this.apiUniqueKey = new Date().getTime().toString();
        this.mobileVerifyService.verifyMobileOtp(this.refId, this.otpLoginForm.controls.mobileOTP.value, this.apiUniqueKey).subscribe(response => {
            this.loading = false;
            if (response['status']) {
                if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                    if (response['payload']['processResponse']['authentication-token']) { // set auth token
...
let MobileVerifyService = class MobileVerifyService {
    constructor(baseAPIService, tokenStorage) {
        this.baseAPIService = baseAPIService;
        this.tokenStorage = tokenStorage;
    }
    getAuthMobile(apiUniqueReqId) {
        let body = {
    ...
    // Mobile OTP verify
    verifyMobileOtp(refid, otp, apiUniqueReqId) {
        let body = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_4__["Constants"].PROJECT_ID,
...
MobileVerifyService.ctorParameters = () => [
    { type: _core_services_base_api_service_service__WEBPACK_IMPORTED_MODULE_3__["BaseAPIService"] },
...
let OtpPageComponent = class OtpPageComponent {
    constructor(otpPageService, tokenStorage, router, route, _snackBar) {
    ...
    submitOtp() {
        ...
        this.otp.toString();
        this.apiUniqueKey = new Date().getTime().toString();
        this.otpPageService.verifyOtp(this.refId, this.otp, this.apiUniqueKey).subscribe(res => {
            if (!res['status']) {
                this.errorPage();
                this.loading = false;
                return;
...
OtpPageComponent.ctorParameters = () => [
...
let OtpPageService = class OtpPageService {
    constructor(baseAPIService, tokenStorage) {
        this.baseAPIService = baseAPIService;
        this.tokenStorage = tokenStorage;
    }
    getAuth(apiUniqueReqId) {
        let body = {
    ...
    verifyOtp(refid, otp, apiUniqueReqId) {
        let body = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_3__["Constants"].PROJECT_ID,
...
let DataService = class DataService {
    constructor(tokenStorage, baseAPIService) {
        this.tokenStorage = tokenStorage;
        this.baseAPIService = baseAPIService;
    }
    getAcceptRejectValue(val) {
        this.accptRejct = val;
    }
...
let AadharCardComponent = class AadharCardComponent {
    constructor() {
        this.hide = true;
...
let DebitCardComponent = class DebitCardComponent {
    constructor(formBuilder, authService, commonFunctions, stepperService, tokenStorage, router, service) {
        this.formBuilder = formBuilder;
        this.authService = authService;
        this.commonFunctions = commonFunctions;
        this.stepperService = stepperService;
    ...
    dcLogin() {
        if (this.stepperService.detect_auth == 1) {
            ...
            this.loading = true;
            this.apiUniqueKey = new Date().getTime().toString();
            this.stepperService.verifyDetails(2, '', '', this.cardNumber, encPass, '', '', this.apiUniqueKey).subscribe(response => {
                this.loading = false;
                if (response['status']) {
    ...
    dcLogin_for_Authorization() {
        ...
        this.loading = true;
        this.apiUniqueKey = new Date().getTime().toString();
        this.stepperService.OtpAuthorization(2, '', '', this.cardNumber, encPass, '', '', this.apiUniqueKey).subscribe(response => {
            this.loading = false;
...
DebitCardComponent.ctorParameters = () => [
...
let OtpLoginComponent = class OtpLoginComponent {
    constructor(service, formBuilder, authService, commonFunctions, stepperService, tokenStorage, router) {
        this.service = service;
        this.formBuilder = formBuilder;
        this.authService = authService;
        this.commonFunctions = commonFunctions;
    ...
    otpLogin() {
        ...
        // If user in Authentication Page i.e when user in links on link
            ...
            if (this.stepperService.otpAuthRefId) {
                this.stepperService.verifyDetails(3, '', '', '', '', this.stepperService.otpAuthRefId, this.otpLoginForm.controls.mobileOTP.value, this.apiUniqueKey).subscribe(response => {
                    this.loading = false;
                    if (response['status']) {
                        if (response['payload']['processResponse']['Error'] == '0' && response['payload']['processResponse']['ErrorCode'] == '200') {
                            if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
    ...
    otpLogin_for_authorization() {
        let apiUniqueKey = new Date().getTime().toString();
        if (this.stepperService.otpAuthRefId) {
            this.stepperService.OtpAuthorization(3, '', '', '', '', this.stepperService.otpAuthRefId, this.otpLoginForm.controls.mobileOTP.value, apiUniqueKey).subscribe(response => {
                this.loading = false;
                if (response['Error'] == '0' && response['ErrorCode'] == '200') {
                    if (response['ProcessVariables']['apiUniqueReqId'] == apiUniqueKey) {
                        this.upload_doc_service();
...
OtpLoginComponent.ctorParameters = () => [
    { type: src_app_services_data_service__WEBPACK_IMPORTED_MODULE_9__["DataService"] },
    { type: _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormBuilder"] },
...
let RetailNetBankingComponent = class RetailNetBankingComponent {
    constructor(service, formBuilder, authService, commonFunctions, stepperService, tokenStorage, router) {
        this.service = service;
        this.formBuilder = formBuilder;
        this.authService = authService;
    ...
    showPassField() {
        ...
        if (this.stepperService.detect_auth == 1) {
            ...
            this.apiUniqueKey = new Date().getTime().toString();
            this.stepperService.auth_reinit(this.apiUniqueKey, 1, this.rnbLoginForm.controls.custId.value).subscribe(response => {
                this.loading = false;
                if (response['status']) {
                    if (response['payload']['processResponse']['Error'] == '0' && response['payload']['processResponse']['ErrorCode'] == '200') {
                        if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                            ...
                            this.authService.alertToUser(src_app_app_constant__WEBPACK_IMPORTED_MODULE_3__["AlertMessages"].SOMETHING_WRONG);
                            this.commonFunctions.showErrorPage();
                        }
                    }
                    else {
    ...
    authorization_RNB() {
        let apiUniqueKey = new Date().getTime().toString();
        if (this.stepperService.otpAuthRefId) {
            this.stepperService.auth_reinit2(apiUniqueKey, 1, this.rnbLoginForm.controls.custId.value).subscribe(response => {
                this.loading = false;
                if (response['Error'] == '0' && response['ErrorCode'] == '200') {
                    if (response['ProcessVariables']['apiUniqueReqId'] == apiUniqueKey) {
    ...
    rnbLogin() {
        ...
        if (this.stepperService.detect_auth == 1) {
            ...
            this.loading = true;
            this.apiUniqueKey = new Date().getTime().toString();
            this.stepperService.verifyDetails(1, this.rnbLoginForm.controls.custId.value, encryptedPassword, '', '', '', '', this.apiUniqueKey).subscribe(response => {
                this.loading = false;
                if (response['status']) {
                    if (response['payload']['processResponse']['Error'] == '0' && response['payload']['processResponse']['ErrorCode'] == '200') {
                        if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
    ...
    rnbLogin_for_authorization() {
        ...
        this.loading = true;
        this.apiUniqueKey = new Date().getTime().toString();
        this.stepperService.OtpAuthorization(1, this.rnbLoginForm.controls.custId.value, encryptedPassword, '', '', '', '', this.apiUniqueKey).subscribe(response => {
            this.loading = false;
            if (response['Error'] == '0' && response['ErrorCode'] == '200') {
                if (response['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                    this.upload_doc_service();
...
RetailNetBankingComponent.ctorParameters = () => [
...
let StepperComponent = class StepperComponent {
    constructor(router, authService, commonFunctions, stepperService, tokenStorage, route) {
        this.router = router;
        this.authService = authService;
        this.commonFunctions = commonFunctions;
        this.stepperService = stepperService;
    ...
    getAuth() {
        this.apiUniqueKey = new Date().getTime().toString();
        this.stepperService.getAuth(this.apiUniqueKey).subscribe(response => {
            this.loading = false;
            if (response['status']) {
                if (response['payload']['processResponse']['Error'] == '0' && response['payload']['processResponse']['ErrorCode'] == '200') {
                    if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
                        if (response['payload']['processResponse']['ProcessVariables']['validMols']) {
                            ...
                            this.authService.alertToUser(_app_constant__WEBPACK_IMPORTED_MODULE_7__["AlertMessages"].SOMETHING_WRONG);
                            this.commonFunctions.showErrorPage();
                        }
                    }
                    else {
    ...
    generateOtp() {
        this.loading = true;
        this.apiUniqueKey = new Date().getTime().toString();
        this.stepperService.auth_reinit(this.apiUniqueKey, 3, '').subscribe(response => {
            this.loading = false;
            if (response['status']) {
                if (response['payload']['processResponse']['Error'] == '0' && response['payload']['processResponse']['ErrorCode'] == '200') {
                    if (response['payload']['processResponse']['ProcessVariables']['apiUniqueReqId'] == this.apiUniqueKey) {
...
let StepperService = class StepperService {
    ...
    auth_reinit(apiUniqueReqId, loginType, iUserId) {
        let body = {
    ...
    auth_reinit2(apiUniqueReqId, loginType, iUserId) {
        let body = {
            "processId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Auth_reinit_otp.PROCESS_ID,
            "workflowId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Auth_reinit_otp.WORKFLOW_ID,
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].PROJECT_ID,
    ...
    verifyDetails(loginType, iUserId, iPass, debitCardNo, debitPIN, refid, otp, apiUniqueReqId) {
        let body = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].PROJECT_ID,
    ...
    OtpAuthorization(loginType, iUserId, iPass, debitCardNo, debitPIN, refid, otp, apiUniqueReqId) {
        let body = {
            "projectId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["Constants"].PROJECT_ID,
            "workflowId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Auth_valid_otp.WORKFLOW_ID,
            "processId": _app_constant__WEBPACK_IMPORTED_MODULE_2__["APIConstants"].Auth_valid_otp.PROCESS_ID,
